...
```
docker run -d --name client-Postgres --publish published=9501,target=80 -e PGADMIN_DEFAULT_PASSWORD=54321 -e PGADMIN_DEFAULT_EMAIL=admin@drupal.com --network net-drupal dpage/pgadmin4
```
...
```
docker run -d --name server-drupal --publish published=9700,target=80 -v vol-drupal-sites:/opt/drupal/web/sites -v vol-drupal-themes:/opt/drupal/web/themes -v vol-drupal-profiles:/opt/drupal/web/profiles -v vol-drupal-modules:/opt/drupal/web/modules --network net-drupal drupal
```
...
![Imagen](diagrama_Contenedores.png)
